    ...
    if ($da_id <> '') {
        ...
        for ($i=0; $i < count($da_id); $i++) { 
            if ($da_id[$i] <> 'on') {
                ...
                $sql = "INSERT INTO qrcodes (da_id, qrcode_img, qrcode_date) VALUES ('$da_id2', '$file.png', DATE(NOW()))";
                $conn->query($sql);
                $sql2 = "UPDATE da_items SET qrcode_status='1' WHERE da_id='$da_id2'";
                // echo $sql . "<br>" . $sql2 . "<br>";
                $conn->query($sql2);
            }
        }
